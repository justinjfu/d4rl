...
class Agent(agent.Agent):
  ...
  def _build_p_loss(self, batch):
    ...
    pol_a_mean, _, log_pi_a_p = self._p_fn(s)
    ...
    info['p_loss'] = p_loss
    info['batch_a0'] = tf.reduce_mean(batch['a1'], axis=0)[0]
    info['batch_a1'] = tf.reduce_mean(batch['a1'], axis=0)[1]
    info['pol_a0'] = tf.reduce_mean(pol_a_mean, axis=0)[0]
    info['pol_a1'] = tf.reduce_mean(pol_a_mean, axis=0)[1]
...
def get_modules(model_params, action_spec):
  def p_net_factory():
    return networks.ConvActorNetwork(
        action_spec,
