...
class ConvActorNetwork(tf.Module):
  """Actor network."""
  ...
  def __init__(
    ...
    super(ConvActorNetwork, self).__init__()
    self._action_spec = action_spec
    self._layers = []
    ...
    self._layers.append(tf.keras.layers.Conv2D(
        filters=4, kernel_size=3, strides=(1, 1), padding='valid', data_format=None,
        dilation_rate=(1, 1), activation='relu', use_bias=True,
    ))
    self._layers.append(tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='valid'))
    self._layers.append(tf.keras.layers.Conv2D(
        filters=4, kernel_size=3, strides=(1, 1), padding='valid', data_format=None,
        dilation_rate=(1, 1), activation='relu', use_bias=True,
    ))
  ...
  def _get_outputs(self, state):
    #NEW
    state = tf.reshape(state, (-1,) + self.img_size)
    h = state
...
class ConvCriticNetwork(tf.Module):
  """Critic Network."""
  ...
  def __init__(
    ...
    super(ConvCriticNetwork, self).__init__()
    ...
    self.conv_layers.append(tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='valid'))
    self.conv_layers.append(tf.keras.layers.Conv2D(
        filters=4, kernel_size=3, strides=(1, 1), padding='valid', data_format=None,
        dilation_rate=(1, 1), activation='relu', use_bias=True,
    ))
  ...
  def __call__(self, state, action):
    ...
    conv_h = state
    for conv_layer in self.conv_layers:
        conv_h = conv_layer(conv_h)
    conv_h = tf.cast(conv_h, dtype=tf.float64)
    ###

    h = tf.concat([conv_h, action], axis=-1)
    for l in self._layers:
      h = l(h)
    return tf.reshape(h, [-1])
